...
exports.WarmupReportService = void 0;
const WarmupHistoryLog_1 = require("../../models/WarmupHistoryLog");
const WarmupMetricsService_1 = require("./WarmupMetricsService");
const WarmupProfileService_1 = require("./WarmupProfileService");
const warmupLogger_1 = require("../utils/warmupLogger");
class WarmupReportService {
    /**
     * Generates a weekly report containing aggregated metrics.
     */
    static async generateWeeklyReport(instanceId, days = 7) {
        warmupLogger_1.warmupLogger.info(`[WarmupReportService] Generating report for instance ${instanceId} (last ${days} days)...`);
        // Fetch basic profile
        ...
        const sentCount = await WarmupHistoryLog_1.WarmupHistoryLog.countDocuments({
            instanceId,
            direction: 'SENT',
            createdAt: { $gte: startDate, $lte: endDate }
        });
        const receivedCount = await WarmupHistoryLog_1.WarmupHistoryLog.countDocuments({
            instanceId,
            direction: 'RECEIVED',
            createdAt: { $gte: startDate, $lte: endDate }
        });
        const aiGeneratedCount = await WarmupHistoryLog_1.WarmupHistoryLog.countDocuments({
            instanceId,
            direction: 'SENT',
            isAiGenerated: true,
            createdAt: { $gte: startDate, $lte: endDate }
...
exports.WarmupReportService = WarmupReportService;
