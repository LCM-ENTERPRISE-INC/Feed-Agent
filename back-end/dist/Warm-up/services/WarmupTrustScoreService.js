...
exports.WarmupTrustScoreService = void 0;
const client_1 = require("@prisma/client");
const warmupLogger_1 = require("../utils/warmupLogger");
const prisma = new client_1.PrismaClient();
class WarmupTrustScoreService {
    /**
     * Calculates the Trust Score (0-100) based on age/phase, blockage history, and asymmetry.
     */
    static async calculateScore(profileId, phase, messagesSentInCurrentBatch, messagesReceivedInCurrentBatch) {
        try {
            let score = 50; // Base Score
            // 1. Age/Phase Bonus
            if (phase === client_1.WarmupPhase.PHASE_2) {
                score += 20;
            }
            else if (phase === client_1.WarmupPhase.PHASE_3) {
                score += 40;
            }
            ...
            warmupLogger_1.warmupLogger.error(`[WarmupTrustScore] Error calculating score for profile ${profileId}`, error);
            // Fallback safe score
            return { score: 50, riskLevel: 'HIGH' };
        }
    }
}
exports.WarmupTrustScoreService = WarmupTrustScoreService;
